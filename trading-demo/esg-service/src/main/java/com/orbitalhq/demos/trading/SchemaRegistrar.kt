...
class SchemaRegistrar(@Value("\${server.port}")
                      ...
                      @Value("\${orbital.host:localhost}")
                      private val orbitalHost: String,
                      @Value("\${service.host:localhost}")
                      private val serviceHost: String): InitializingBean {
   override fun afterPropertiesSet() {
      logger.info { "Orbital host => $orbitalHost, service host => $serviceHost" }
      val appName = "trading-demo-service"
      val publisher = SchemaPublisherService(
         appName,
         RSocketSchemaPublisherTransport(
            TcpAddress(orbitalHost, 7655)
         )
      )
      publisher.publish(
         PackageMetadata.from("com.lunarbank.demo", appName),
         SpringTaxiGenerator.forBaseUrl("http://$serviceHost:${serverPort}")
